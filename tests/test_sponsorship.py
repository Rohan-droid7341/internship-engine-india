...
class TestCitizensOnly:
    def test_citizenship_required(self):
        assert (
            sponsorship.classify("Applicants must be a U.S. citizen due to contract requirements.")
            == "citizens-only"
        )

    def test_clearance(self):
        assert (
            sponsorship.classify("Candidates need an active TS/SCI clearance to start.")
            == "citizens-only"
        )
        assert (
            sponsorship.classify("Ability to obtain a security clearance is required.")
            == "citizens-only"
        )

    def test_itar(self):
        assert (
            sponsorship.classify("This position is subject to ITAR regulations.") == "citizens-only"
        )

    def test_citizens_or_green_card(self):
        assert (
            sponsorship.classify("Open to U.S. citizens and lawful permanent residents only.")
            == "citizens-only"
        )
...
class TestNoSponsorship:
    ...
    def test_does_not_offer(self):
        assert (
            sponsorship.classify("Please note this role does not offer visa sponsorship.")
            == "no-sponsorship"
        )
    ...
    def test_not_available(self):
        assert (
            sponsorship.classify("Immigration sponsorship is not available for this position.")
            == "no-sponsorship"
        )
...
class TestOffers:
    def test_sponsorship_available(self):
        assert (
            sponsorship.classify("Visa sponsorship is available for exceptional candidates.")
            == "offers"
        )
...
class TestUnknownAndTraps:
    ...
    def test_plain_role_text(self):
        assert (
            sponsorship.classify("Build distributed systems in Go and Python. Free lunch.")
            == "unknown"
        )
