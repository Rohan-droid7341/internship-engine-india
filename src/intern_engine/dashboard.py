...
def _rows(open_jobs: list[dict]) -> str:
    ...
    for r in open_jobs:
        ...
        rows.append(
            ...
            f"<td>{escape(r.get('title', ''))} {chips}</td>"
            f"<td>{cycle_tag}</td>"
...
def generate(store_data: dict, stats: dict) -> None:
    ...
    if config.want_india(cfg) and config.want_remote(cfg):
        region = "India & Verified Remote"
    elif config.want_india(cfg):
        region = "India"
    elif config.include_international(cfg):
...
<title>Indian Tech Internships - Live Dashboard</title>
<meta name="description" content="Summer 2027 & Fall 2026 Indian & Remote tech internships, refreshed every hour. Verified stipends, direct career feeds, and daily alerts.">
<meta property="og:title" content="Indian Tech Internships - Live Dashboard">
<meta property="og:description" content="{len(open_jobs)} open tech internships in India and remote, refreshed every hour. Direct company feeds and verified stipends.">
...
<style>
  ...
  * {{ box-sizing:border-box; }}
  ...
  <th>Location</th><th>Pay / Stipend</th><th>Posted</th><th></th></tr></thead>
...
(function () {{
  ...
  [q, cycle, cat, age, stated].forEach(function (el) {{
